...
(define (p) (p));              Здесь интерпретатор зацикливается!
(define (test x y)
  (if (= x 0)
      0
      y))
...
; При аппликативном порядке вычислений сначала полностью вычисляются операнды, а
; затем происходит вычисление процедуры с вычисленными значениями операндов.
; То есть при вычислении в случае АПЛИКАТИВНОГО порядка вычисления мы не войдем
; в процедуру test так-как не сможем вычислить рекурсивную процедуру (p)

; При нормальном порядке операнды вычисляются не сразу, а лишь по мере необходимости
; при использовании в теле процедуры. Поэтому сначала вычисляется предикат if, он
; оказывается истинным и потому функция возвращает 0, так и не столкнувшись с
; необходимостью вычислять (p).
